...
public class LocationUtils {
	...
	public static Boolean doesPersonBelongToGivenLocations(Person person, PersonAttributeType personAttributeType,
			List<String> sessionLocationsLists) {
		PersonAttribute personAttribute = person.getAttribute(personAttributeType);
	...
	/**
	 * Used to get the accessible locations for the user. It will first get from the
	 * user property. if the user property is not available, then check for the
	 * session location. If both are not available then return null.
	 * 
	 ...
	 * Used to get the accessible locations for the user. It will first get from the
	 * user property. if the user property is not available, then check for the
	 * locations having user privilege. If both are not available then return null.
	 * 
	 * @param authenticatedUser Authenticated user
	...
	public static void createPersonAttributeTypeForLocation() {
		if (null == Context.getPersonService()
				.getPersonAttributeTypeByUuid(LocationBasedAccessConstants.PERSONATTRIBUTETYPE_UUID)) {
	...
	public static LocationBasedAccessHeuristicsService getHeuristicsService() {
		return (LocationBasedAccessHeuristicsService) Context.getService(LocationBasedAccessHeuristicsService.class);
	}
}
